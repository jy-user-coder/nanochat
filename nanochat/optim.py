...
class MuonAdamW(torch.optim.Optimizer):
    ...
    def estimate_collective_bus_bytes(self) -> float:
        """Single-GPU optimizer: no collective communication."""
        return 0.0
...
class DistMuonAdamW(torch.optim.Optimizer):
    ...
    def estimate_collective_bus_bytes(self) -> float:
        """
        Estimate application-visible collective bus-bytes for one optimizer step.

        This is an approximation of the collective traffic generated by this
        optimizer's all-reduce / reduce-scatter / all-gather pattern.
        """
        ...
        # all_reduce ~= 2*(N-1)/N * payload_bytes
        # reduce_scatter + all_gather has the same aggregate bus-byte factor
        collective_factor = 2.0 * (world_size - 1) / world_size
        payload_bytes = 0

        for group in self.param_groups:
            if group['kind'] == 'adamw':
                payload_bytes += sum(p.numel() * p.element_size() for p in group['params'])
            elif group['kind'] == 'muon':
                params = group['params']
                if not params:
                    continue
                chunk_size = (len(params) + world_size - 1) // world_size
                padded_num_params = chunk_size * world_size
                payload_bytes += padded_num_params * params[0].numel() * params[0].element_size()
            else:
                raise ValueError(f"Unknown optimizer kind: {group['kind']}")

        return collective_factor * payload_bytes
